...
// Check if 'au_num' is passed via URL
$au_num = isset($_GET['au_num']) ? $_GET['au_num'] : null;

// Fetch author ID based on 'au_num'
if ($au_num) {
    $author_sql = "SELECT au_id FROM author WHERE au_num = ?";
    $stmt = $conn->prepare($author_sql);
    $stmt->bind_param("s", $au_num);
    $stmt->execute();
    $author_result = $stmt->get_result();

    if ($author_result->num_rows > 0) {
        $author_row = $author_result->fetch_assoc();
        $author_id = $author_row['au_id'];
    } else {
                            ...
                            <article class="col-md-12">
                                <div class="cards-1">
                                    <div class="container">
                                        <div class="row">
                                            <?php
                                            // Fetch books by author or all books if no author specified
                                            ...
                                            if ($result->num_rows > 0) {
                                                ...
                                                echo "<p class='text-center'>No books available for this author.</p>";
                                            }
